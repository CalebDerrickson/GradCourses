...
\begin{solution}
    ...
    \begin{itemize}[-]
        ...
        \begin{align*}
            ...
            \implies 1 &< \frac{1}{|\lm|} &\text{(Rearranging.)}\\
            \implies 1 &> |\lm| &\text{(Taking Reciprocal.)}
        \end{align*}
        \vspace{-6mm}\alignbreak
        Thus, we have shown that all eigenvalues of $(D - L)\inv U$ have magnitude less than 1\footnote{The case where $\lm = 0$ implies that $x \in \ker U$. }, so $T$ is then a contraction mapping. Therefore by the Contraction Mapping theorem, there exists a fixed point $x$ of the mapping such that $Tx = x$. This then implies the following chain: 
